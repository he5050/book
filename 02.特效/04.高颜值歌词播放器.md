---
date: 2025-09-26 18:10:59
title: 高颜值歌词播放器
permalink: /pages/13ed70
categories:
  - 特效
---
...
这是一个基于 React 实现的高颜值歌词播放器组件，具有玻璃拟态设计风格和流畅的动画效果。

## 效果展示

![歌词播放器效果](https://picsum.photos/id/33/1920/1080)
		...
		if (activeIndex !== currentActiveIndex) {
			...
			if (activeIndex >= 0 && lyricLineRefs.current[activeIndex] && lyricContainerRef.current) {
				...
				if (line) {
					...
					const scrollTop =
						line.offsetTop - container.offsetTop - containerRect.height / 2 + lineRect.height / 2;
					container.scrollTo({
						top: scrollTop,
						behavior: 'smooth'
					});
...
3. **自动滚动**：使用 `scrollTo` 方法使当前歌词行居中显示
