<template>
  ...
  const formattedDate = computed(() => {
    return `${dayjs().format('YYYY-MM-DD')}(${timeNow.ncWeek}), 农历:${timeNow.Animal}年,${timeNow.IMonthCn}${timeNow.IDayCn}(${timeNow.gzYear}年${timeNow.gzMonth}月${timeNow.gzDay}日)`;
  });
  ...
  const getWeather = async () => {
    try {
      ...
      const response = await fetch(`http://v1.yiketianqi.com/free/week?appid=18979323&appsecret=iIfhjL3K`);
