...
const BrowserInfoDemo: React.FC = () => {
  ...
  const { locationInfo, loading: locationLoading, error: locationError, refresh: refreshLocation, clearCache } = useLocation();
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mt-8 bg-yellow-50 rounded-lg p-6 border border-yellow-200">
        ...
        <ul className="text-sm text-yellow-700 space-y-2">
          <li>• <strong>浏览器信息</strong>：通过解析 navigator.userAgent 获取浏览器类型、版本和操作系统信息</li>
          <li>• <strong>IP地址</strong>：使用 WebRTC 获取本地IP，通过第三方API获取公网IP</li>
          <li>• <strong>位置信息</strong>：基于IP地址通过第三方地理位置API获取，支持缓存以减少请求次数</li>
          <li>• <strong>缓存机制</strong>：位置信息会缓存1小时，避免频繁请求第三方服务</li>
          <li>• <strong>隐私保护</strong>：所有信息仅在客户端获取和显示，不会上传到服务器</li>
        </ul>
      </div>
    </div>
  );
