...
const BeautifulFlower: React.FC<BeautifulFlowerConfig> = ({
	...
	useEffect(() => {
		if (containerRef.current) {
			...
			container.style.setProperty('--animation-speed', internalConfig.animationSpeed.toString());
			container.style.setProperty('--flower-color-1', internalConfig.flowerColors[0]);
			container.style.setProperty(
				'--flower-color-2',
				internalConfig.flowerColors[1] || internalConfig.flowerColors[0]
			);
			container.style.setProperty('--stem-color', internalConfig.stemColor);
			container.style.setProperty('--grass-color', internalConfig.grassColor);
			container.style.setProperty('--light-color-1', internalConfig.lightColors[0]);
			container.style.setProperty(
				'--light-color-2',
				internalConfig.lightColors[1] || internalConfig.lightColors[0]
			);
			container.style.setProperty('--bg-color', internalConfig.backgroundDark ? '#000' : '#87ceeb');
		}
	}, [internalConfig]);
	...
	const handleConfigChange = (key: string, value: any) => {
		...
		// 特殊处理颜色数组
		if (key === 'flowerColors' && !Array.isArray(value)) {
			newConfig.flowerColors = [value, newConfig.flowerColors[1] || value];
		}
		if (key === 'lightColors' && !Array.isArray(value)) {
			newConfig.lightColors = [value, newConfig.lightColors[1] || value];
		}
		...
		<div className="beautiful-flower" ref={containerRef}>
			...
			>
				...
				<div className="flowers">
					{Array.from({ length: Math.min(internalConfig.flowerCount, 3) }, (_, i) =>
						renderFlower(i)
					)}
					{renderGrass()}
				...
				<div className="config-panel">
					...
					<div className="config-item">
						...
						<input
							...
							onChange={e =>
								handleConfigChange('flowerColors', [
									e.target.value,
									internalConfig.flowerColors[1] || e.target.value
								])
							}
						/>
					</div>
					...
					<div className="config-item">
						...
						<input
							...
							onChange={e =>
								handleConfigChange('lightColors', [
									e.target.value,
									internalConfig.lightColors[1] || e.target.value
								])
							}
						/>
					</div>
