...
export default function MdWatermark(props: WatermarkProps) {
	...
	function drawRotatedText(
		...
		lines.forEach((line, index) => {
			const yPos =
				-(totalTextHeight / 2) + index * scaledLineHeight * ratio + (scaledLineHeight * ratio) / 2;
			ctx.fillText(line, 0, yPos);
		});
		ctx.restore();
	}

	// 优化: 缓存backgroundPosition，避免重复计算
	const backgroundPosition = useMemo(() => {
		const backgroundPositionX = align === 'left' ? 'left' : align === 'right' ? 'right' : 'center';

		const backgroundPositionY = align === 'top' ? 'top' : align === 'bottom' ? 'bottom' : 'center';
		// 修正: 用 offset.y
		return `${backgroundPositionX} ${offset.x}px, ${backgroundPositionY} ${offset.y}px`;
	}, [align, offset.x, offset.y]);
	...
	const drawWatermarkCallback = useCallback(() => {
		...
		if (imageUrl && isImageLoaded && !imageError) {
			...
			img.onload = () => {
				...
				if (ctx) {
					...
					drawRotatedImage(
						ctx,
						img,
						canvasToUse.width / 4 + offsetX + userOffsetX,
						canvasToUse.height / 4 + offsetY + userOffsetY,
						angleRadians,
						scaledWidth,
						scaledHeight
					);
					drawRotatedImage(
						ctx,
						img,
						(canvasToUse.width * 3) / 4 + offsetX + userOffsetX,
						(canvasToUse.height * 3) / 4 + offsetY + userOffsetY,
						angleRadians,
						scaledWidth,
						scaledHeight
					);
					...
					try {
						...
						if (optimizePerformance && getCanvasContext(canvas)) {
							canvas.width = canvasToUse.width;
							canvas.height = canvasToUse.height;
							getCanvasContext(canvas)?.drawImage(canvasToUse, 0, 0);
						}
			...
			const rotatedTextWidth =
				Math.abs(maxTextWidth * Math.cos(angleRadians)) +
				Math.abs(totalTextHeight * Math.sin(angleRadians));
			const rotatedTextHeight =
				Math.abs(maxTextWidth * Math.sin(angleRadians)) +
				Math.abs(totalTextHeight * Math.cos(angleRadians));
			...
			if (ctx) {
				...
				drawRotatedText(
					ctx,
					lines,
					tileWidth / 4 + offsetX + userOffsetX,
					tileHeight / 4 + offsetY + userOffsetY,
					angleRadians,
					totalTextHeight,
					scaledLineHeight,
					ratio
				...
				drawRotatedText(
					ctx,
					lines,
					(tileWidth * 3) / 4 + offsetX + userOffsetX,
					(tileHeight * 3) / 4 + offsetY + userOffsetY,
					angleRadians,
					totalTextHeight,
					scaledLineHeight,
					ratio
		...
		optimizePerformance,
		generateCacheKey,
		getWatermarkFromCache,
		saveWatermarkToCache,
	...
	const startFadeAnimation = useCallback(() => {
		...
		// 创建动画
		watermarkLayer.animate([{ opacity: '0' }, { opacity: opacity.toString() }, { opacity: '0' }], {
	...
	const stopAnimation = useCallback(() => {
		...
		setRotationAngle(0); // 重置旋转角度

		const watermarkLayer = watermarkLayerRef.current;
		if (watermarkLayer) {
			// 停止所有正在运行的CSS动画
			const animations = watermarkLayer.getAnimations();
			animations.forEach(animation => animation.cancel());

			// 重置样式
			watermarkLayer.style.opacity = opacity.toString();
			watermarkLayer.style.transform = '';
			watermarkLayer.style.transition = 'none'; // 防止过渡效果

			// 强制重绘以确保样式立即应用
			void watermarkLayer.offsetHeight;
			watermarkLayer.style.transition = '';
		}
	}, [opacity]);

	// 优化: 合并动画初始化为单一函数，减少重复逻辑
	...
	// 监听容器尺寸变化的副作用
	useEffect(() => {
		if (!responsiveOptions.responsive) return;
	...
	}, [container, responsiveOptions.responsive]);
