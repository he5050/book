...
<demo react="react/InfiniteScroll/index.tsx" 
:reactFiles="['react/InfiniteScroll/index.tsx','react/InfiniteScroll/index.scss']" 
/>
...
const InfiniteScrollExample: React.FC<InfiniteScrollExampleProps> = ({
	...
	const [items, setItems] = useState<number[]>(Array.from({ length: initialCount }, (_, i) => i));
	const [hasMore, setHasMore] = useState(true);
	...
	const fetchMoreData = () => {
		...
		setTimeout(() => {
			...
			const newItems = Array.from({ length: loadCount }, (_, i) => currentLength + i);
						...
						<div key={index} className="item">
							<div className="item-content">
								...
								<div className="item-text">
									<div className="item-title">数据项 #{index}</div>
									<div className="item-description">这是第 {index + 1} 个数据项的描述信息</div>
								</div>
							</div>
						</div>
					))}
...
```mermaid
graph LR
    A[实现方式] --> B[react-infinite-scroll-component]
    A --> C[rc-virtual-list]
    A --> D[自定义实现]
    A --> E[Intersection Observer]
