...
permalink: /examples/copy-functionality
...
- **多种方案**: 支持三种不同的复制实现方式
- **兼容性考虑**: 从传统方案到现代API的完整覆盖
- **格式支持**: 支持纯文本、HTML和图片复制
- **用户体验**: 提供即时反馈和错误处理

### 技术特性

- **渐进增强**: 优先使用现代API，降级到传统方案
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `supportHtml` | boolean | false | 是否支持HTML复制 |
...
**问题**: 在HTTP环境下clipboard API不可用
**解决方案**:
- 开发环境使用localhost（被视为安全上下文）
- 生产环境部署HTTPS
- 提供execCommand降级方案
