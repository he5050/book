...
2. **暂停动画**：当页面不可见时，暂停不必要的动画效果，减少CPU使用
...
- 所有现代浏览器都支持此API
- 在移动设备上，当浏览器最小化或切换到其他应用时，也会触发visibilitychange事件
- 不要完全依赖此API来实现安全功能，因为它可以被恶意脚本覆盖
...
- 设备电量低时，系统可能会忽略唤醒锁请求
- 目前主要在Chrome和基于Chromium的浏览器中支持
- 使用时应考虑电池消耗，仅在必要时启用
- 页面可见性变化时需要重新请求唤醒锁

## Broadcast Channel API

Broadcast Channel API 允许同源的不同浏览上下文（如不同标签页、iframe或worker）之间进行通信。
...
- 大多数现代浏览器都支持此API，但IE不支持
- 消息是广播的，所有监听同一频道的上下文都会收到消息
- 当不再需要通信时，应调用`close()`方法释放资源
- 相比于其他通信方式（如LocalStorage事件），Broadcast Channel API更可靠、更高效

## PerformanceObserver API

PerformanceObserver API 允许监测性能度量事件，在浏览器的性能时间轴记录新的performance entry时通知开发者。
...
1. **性能监控**：监控页面加载、资源加载、JavaScript执行等性能指标
2. **用户体验分析**：收集真实用户的性能数据，分析用户体验
3. **性能预算监控**：设置性能预算，当性能指标超出预期时发出警告
4. **自定义性能测量**：使用mark和measure测量特定代码块的执行时间
...
// 模拟一个工作单元
let workUnits = Array(50).fill().map((_, i) => i);
...
- Chrome、Firefox和Edge支持此API，但Safari不支持
- 回调函数接收一个`IdleDeadline`对象，包含`timeRemaining()`方法和`didTimeout`属性
- `timeRemaining()`返回的时间以毫秒为单位，通常在0-50ms之间
- 不要在回调中执行DOM操作，因为可能会触发重新渲染
- 对于需要在下一帧之前完成的任务，应使用`requestAnimationFrame`而非`requestIdleCallback`
- 可以使用polyfill在不支持的浏览器中模拟此功能
- 设置`timeout`选项可确保任务最终会执行，即使浏览器一直很忙

## IntersectionObserver API

IntersectionObserver API 提供了一种异步检测目标元素与祖先元素或视口相交状态的方法，可用于实现懒加载、无限滚动等功能。
...
1. **图片懒加载**：仅在图片进入视口时才加载图片
2. **无限滚动**：当用户滚动到页面底部时加载更多内容
3. **内容可见性跟踪**：跟踪内容是否对用户可见，用于分析或广告曝光统计
4. **动画触发**：当元素进入视口时触发动画
5. **视频自动播放/暂停**：当视频进入/离开视口时自动播放/暂停

### 注意事项和兼容性

- 所有现代浏览器都支持此API，但IE不支持
- 回调函数在主线程上异步执行，不会阻塞主线程
- `threshold`可以是单个数值或数值数组，表示触发回调的可见性百分比
- `rootMargin`类似于CSS的margin，可以扩大或缩小根元素的判定范围
- 当目标元素不再需要观察时，应调用`unobserve()`或`disconnect()`方法释放资源
- 可以使用polyfill在不支持的浏览器中模拟此功能
- 相比于传统的滚动事件监听，IntersectionObserver更高效，不会导致页面卡顿

## ResizeObserver API

ResizeObserver API 提供了一种高效的方式来监视元素大小的变化，而不会触发无限循环的回调或导致性能问题。
...
const resizeObserver = new ResizeObserver(entries => {
  for (const entry of entries) {
    ...
    console.log('元素大小已改变:');
    console.log(`宽度: ${contentRect.width}`);
    console.log(`高度: ${contentRect.height}`);
    
    // 根据新尺寸调整元素样式
    if (contentRect.width < 200) {
    ...
    console.log(`内容盒宽度: ${contentBoxSize.inlineSize}`);
    console.log(`内容盒高度: ${contentBoxSize.blockSize}`);
  }
});

// 开始观察元素
const element = document.querySelector('#resizeMe');
...
1. **响应式组件**：根据容器大小调整组件布局或样式
2. **自适应图表**：根据容器大小调整图表尺寸和布局
3. **自定义滚动条**：根据内容大小调整自定义滚动条
4. **文本溢出处理**：检测文本容器大小变化，处理文本溢出
5. **网格布局**：实现动态调整的网格布局系统

### 注意事项和兼容性

- 所有现代浏览器都支持此API，但IE不支持
- 回调函数在动画帧中批量异步执行，不会导致性能问题
- 避免在回调中直接修改被观察元素的大小，可能导致无限循环
- `contentBoxSize`、`borderBoxSize`和`devicePixelContentBoxSize`提供了不同盒模型的尺寸信息
- 对于旧版浏览器，可以使用`contentRect`属性获取基本尺寸信息
- 相比于`window.resize`事件或CSS媒体查询，ResizeObserver更精确且性能更好
- 可以使用polyfill在不支持的浏览器中模拟此功能
...
3. **UI更新**：根据优先级调度不同部分的UI更新
4. **资源加载**：根据重要性调度资源加载任务
5. **定时任务**：替代setTimeout实现更可靠的定时任务

### 注意事项和兼容性

- 目前主要在Chrome 94+和基于Chromium的浏览器中支持
- 任务优先级分为三级：`'user-blocking'`（最高）、`'user-visible'`（默认）和`'background'`（最低）
- 返回Promise对象，可以使用then/catch处理任务完成或失败
- 可以使用AbortController取消尚未执行的任务
- delay参数允许设置任务延迟执行的时间（毫秒）
- 相比于setTimeout，scheduler.postTask提供了更精确的任务调度和优先级控制
- 在不支持的浏览器中，可以使用polyfill或回退到setTimeout

## AbortController API

AbortController API 提供了一种标准方式来取消一个或多个异步操作，如fetch请求、动画或任何其他可取消的操作。
...
1. **取消网络请求**：当用户导航离开页面或取消操作时，终止进行中的fetch请求
...
- 被取消的Promise会以AbortError拒绝，应在catch中处理
- 可以使用`signal.addEventListener('abort', callback)`监听取消事件
- `signal.aborted`属性表示信号是否已被取消
- 对于不支持AbortController的旧版浏览器，可以使用polyfill
- 取消HTTP请求时，虽然客户端不再等待响应，但服务器可能仍会完成请求处理
...
- 需要在安全上下文（HTTPS）中使用
- 目前主要在Chrome 86+和基于Chromium的浏览器（如Edge）中支持
...
- 文件操作是异步的，需要使用Promise或async/await处理
