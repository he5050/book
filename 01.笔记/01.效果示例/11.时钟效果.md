...
const AnalogClock: React.FC<AnalogClockProps> = ({
  ...
  const getAngles = (date: Date) => {
    ...
    // 计算角度（12点为0度，顺时针为正）
    const secondAngle = smoothAnimation 
...
*时钟效果不仅是时间的展示，更是动画技术和用户体验设计的完美结合。* ⏰
